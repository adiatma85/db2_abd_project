    ...
    RETURNS TABLE (
        ...
        DROPOFF_LOC VARCHAR(80)
            ...
            SELECT CONCAT(CONCAT(c.first_name, ", "), c.last_name) AS "Customer Name",
                ...
                FROM
                CAR_RENTAL_SCHEMA.rental AS r
                JOIN
                CAR_RENTAL_SCHEMA.customer AS c ON r.customer_id = c.id
                JOIN
                CAR_RENTAL_SCHEMA.vehicle_type AS vt ON r.vehicle_type_id = vt.id
                JOIN
                CAR_RENTAL_SCHEMA.fuel_option AS fo ON r.fuel_option_id = fo.id
                JOIN
                CAR_RENTAL_SCHEMA.location AS l1 ON r.pickup_location_id = l1.id
                JOIN
                CAR_RENTAL_SCHEMA.location AS l2 ON r.drop_off_location_id = l2.id
                WHERE
